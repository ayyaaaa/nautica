    ...
    <div className="min-h-screen bg-muted/20 flex flex-col md:flex-row">
      ...
      <aside className="w-64 bg-background border-r hidden md:flex flex-col fixed h-full z-20 shadow-sm">
        ...
        <nav className="flex-1 p-4 space-y-1">
          <SidebarItem href="/portal" icon={<LayoutDashboard />} label="Dashboard" />
          <SidebarItem href="/register" icon={<Ship />} label="Register Vessel" />

          <p className="px-4 text-xs font-semibold text-muted-foreground mb-2 mt-6 uppercase tracking-wider">
            Manage
          </p>
          {/* THIS IS THE BUTTON YOU NEEDED */}
          <SidebarItem href="/portal/services" icon={<Wrench />} label="Service Requests" />
          {/* Future: Payment History */}
          {/* <SidebarItem href="/portal/payments" icon={<CreditCard />} label="My Payments" /> */}
        </nav>
      ...
      <div className="md:hidden bg-background border-b p-4 flex items-center justify-between sticky top-0 z-30">
        <div className="flex items-center gap-2 font-bold text-lg text-primary">
          <Anchor className="h-5 w-5" /> My Harbor
        </div>
